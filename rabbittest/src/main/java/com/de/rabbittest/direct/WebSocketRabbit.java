 ...
 *  
 */
@ServerEndpoint(value = "/webSocketRabbit")
@Component
@Slf4j
public class WebSocketRabbit {
    /**
     ...
     */
    private static ConcurrentHashMap<String, WebSocketRabbit> webSocketMap = new ConcurrentHashMap<>();
    /**
    ...
    public static void sendAllByBinary(ByteBuffer byteBuffer) {
        if (!webSocketMap.isEmpty()) {
            Collection<WebSocketRabbit> values = webSocketMap.values();
            for (WebSocketRabbit next : values) {
